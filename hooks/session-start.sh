...
if [[ -n "$TOOLKIT_ROOT" && -d "$TOOLKIT_ROOT/hooks" ]]; then
    ...
    for _hook in check-inbox.sh checklist-reminder.sh contribution-detector.sh done-sound.sh session-start.sh sync.sh title-update.sh todo-capture.sh tool-router.sh worktree-guard.sh write-guard.sh; do
        ...
        _source="$TOOLKIT_ROOT/hooks/$_hook"
    ...
    # Check statusline
    if [[ -f "$CLAUDE_DIR/statusline.sh" && ! -L "$CLAUDE_DIR/statusline.sh" && -f "$TOOLKIT_ROOT/hooks/statusline.sh" ]]; then
        if diff -q "$CLAUDE_DIR/statusline.sh" "$TOOLKIT_ROOT/hooks/statusline.sh" >/dev/null 2>&1; then
            ln -sf "$TOOLKIT_ROOT/hooks/statusline.sh" "$CLAUDE_DIR/statusline.sh" 2>/dev/null && _REPAIRED="$_REPAIRED statusline.sh"
        else
    ...
    _bg_sync_health() {
        ...
        if [[ -d "$CLAUDE_DIR/skills" ]]; then
            ...
            for _SKILL_DIR in "$CLAUDE_DIR"/skills/*/; do
                ...
                # Skill ownership check: after decomposition, canonical skills live under
                # $TOOLKIT_ROOT/skills/ (core-only: setup-wizard, remote-setup) and in
                # sibling destinclaude-* plugin dirs. Anything matching those is toolkit-managed.
                if [[ -n "$TOOLKIT_ROOT" && -d "$TOOLKIT_ROOT/skills/$_SKILL_NAME" ]]; then
                    continue
                fi
                if [[ -d "$HOME/.claude/plugins/destinclaude-${_SKILL_NAME}" ]]; then
                    continue
                fi
...
if command -v node &>/dev/null; then
    ...
    ANNOUNCEMENT_FETCH=""
    [[ -n "$TOOLKIT_ROOT" ]] && ANNOUNCEMENT_FETCH="$TOOLKIT_ROOT/hooks/announcement-fetch.js"
...
if command -v node &>/dev/null; then
    _DESTINTIP_CATALOG=""
    [[ -n "$TOOLKIT_ROOT" ]] && _DESTINTIP_CATALOG="$TOOLKIT_ROOT/data/destintip-catalog.json"
